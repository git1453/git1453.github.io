# Git Cheatsheet Git 备忘单
[原文](https://cs.fyi/guide/git-cheatsheet)

Everything You Need to Know in One Handy Cheatsheet
您需要了解的所有内容都在一张方便的备忘单中
...
Git 是一个版本控制系统，允许您跟踪对文件和文件夹的更改。它是一个功能强大的工具，可用于从小型个人项目到大型企业应用程序的所有内容。
...
本指南是对最常见 Git 命令的快速参考。它不是 Git 的综合指南，而是对最常见命令的快速参考。

Here's the revised git cheatsheet with improved section titles and reorganized:
以下是修订后的 git 备忘单，改进了章节标题并进行了重新组织：
...
git reflog
```
##Submodules, Subtrees, and Advanced Submodules子模块、子树和高级子模块
```git
...
```
## Hooks and Automation, and Diff and Merge Tools挂钩和自动化，以及比较和合并工具
```git
...
git grep <pattern>
```
## Bisecting, Debugging, and Performance Issues平分、调试和性能问题
```git
